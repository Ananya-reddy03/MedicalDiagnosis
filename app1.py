...
background_image_url = "https://img.freepik.com/premium-photo/medical-abstract-healthcare-networking-background-creating-using-generative-ai-tools_852340-141.jpg?w=2000"  # Replace with your image URL
...
width: 100%;
height: 100%;
background-color: rgba(0, 0, 0, 0.3);  /* Adjust opacity to brighten */
}}
...
if selected == 'Alzheimers Prediction':
    ...
    st.write("Enter the following details to predict Alzheimer's disease:")

    # Collect input from user
    age = display_input('Age', 'Enter the age of the person', 'age', 'number')
    gender = display_input('Gender (1 = Male; 0 = Female)', 'Enter gender (1 for Male, 0 for Female)', 'gender', 'number')
    education_level = display_input('Education Level (Years)', 'Enter education level', 'education_level', 'number')
    bmi = display_input('BMI', 'Enter BMI', 'bmi', 'number')
    smoking = display_input('Smoking (1 = Yes; 0 = No)', 'Smoking status', 'smoking', 'number')
    # Add other inputs...

    if st.button('Alzheimer\'s Test Result'):
        try:
            # Create input DataFrame
            input_data = pd.DataFrame([[age, gender, education_level, bmi, smoking]],
                                      columns=['Age', 'Gender', 'EducationLevel', 'BMI', 'Smoking'])

            # Load training columns
            with open('alzheimers_features.pkl', 'rb') as f:
            ...
            with open('alzheimers_scaler.pkl', 'rb') as f:
                scaler = pickle.load(f)
            input_data_scaled = scaler.transform(input_data)

            # Load the model and make predictions
            with open('alzheimers_model.sav', 'rb') as f:
                alzheimers_model = pickle.load(f)
            prediction = alzheimers_model.predict(input_data_scaled)

            diagnosis = 'The person has Alzheimer\'s disease' if prediction[0] == 1 else 'The person does not have Alzheimer\'s disease'
